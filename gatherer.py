...
def find_files_by_regex(sn_regex, dir_locations) -> List[SNReportFile]:
  ...
  for dir_location in dir_locations:
    ...
    try: files = [ f for f in os.listdir(dir_location) if os.path.isfile(os.path.join(dir_location, f)) ]
      ...
      logging.warning(f"There was an issue gathering files at location {dir_location}:\n{str(e)}")
      pass
    ...
    for f in files:
      if sn_regex.search(f):
        ...
        try: result.append(SNReportFile(f, path, os.path.getmtime(path)))
          ...
          logging.warning(f"There was an issue retrieving nodified time for file at location {path}:\n{str(e)}")
          pass
...
def copy_files(file_paths, target_directory):
  ...
  try: os.mkdir(dest)
    ...
    logging.warning(f"There was an issue creating directory '{dest}':\n{str(e)}")
    pass

  for f in file_paths:
    try: shutil.copy(f, dest)
    except Exception as e: 
      logging.warning(f"There was an issue copying file '{f}' to location '{dest}':\n{str(e)}")
      pass
...
def test_html_file_content(report_path: str, tests_object) -> List[SNReportTest]:
  ...
  try: 
    ...
    logging.warning(f"There was an issue testing html content of file '{f}':\n{str(e)}")
    pass
